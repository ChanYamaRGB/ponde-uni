...
for (const folder of commandFolders) {
    ...
    for (const file of commandFiles) {
        ...
        const command = module.default; // 【修正】.mjsのデフォルトエクスポートを取得
...
const handlerFiles = fs.readdirSync(handlersPath).filter(file => !fs.statSync(path.join(handlersPath, file)).isDirectory() && file.endsWith('.mjs'));

for (const file of handlerFiles) {
    const filePath = path.join(handlersPath, file);
    const fileUrl = pathToFileURL(filePath).href;
    const module = await import(fileUrl);
    const event = module.default; // 【修正】
...
server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
        console.warn(`Koyeb Log: Port ${PORT} is already in use.`);
    }
});
